...
function Card(props) {
  ...
  const deleteButtonClassName = (`photocards__delete-button button ${isOwn ? 'photocards__delete-button__visible': ''}`);
  const isLiked = props.card.likes.some(i => i._id === currentUser._id);
  const cardLikeButtonClassName=(`photocards__likeimg ${isLiked ? 'photocards__likeimg-fill':''}`);
    ...
    <div>
      <div className="photocards__item">
        ...
        <div className="photocards__name-like-area">
          ...
          <button type="button" onClick={handleLikeClick} className="photocards__like-button  button">
